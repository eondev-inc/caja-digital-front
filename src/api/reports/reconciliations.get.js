...
/**
 * Obtiene el reporte de cuadraturas de caja como blob PDF.
 *
 * @param {Object} params
 ...
 */
export const getReconciliationsReport = async ({ startDate, endDate, userId } = {}) => {
  try {
    const params = { startDate, endDate };
    if (userId) params.userId = userId;

    const response = await axiosInstance.get('/reports/reconciliations', {
      params,
      responseType: 'blob',
      timeout: 30000,
    });
    ...
    console.error('Error al generar reporte de cuadraturas:', error);
    const errorMessage =
